...
from sqlalchemy import Boolean, Date, DateTime, Float, ForeignKey, Index, Integer, Text, UniqueConstraint, func
...
class CreatorIntelligenceReportRecord(Base):
    """Immutable Creator Intelligence Report generated by one analysis job."""

    __tablename__ = "creator_intelligence_reports"
    __table_args__ = (
        Index("ix_creator_intelligence_reports_account_created", "account_id", "created_at"),
        Index("ix_creator_intelligence_reports_status", "report_status"),
    )

    analysis_job_id: Mapped[str] = mapped_column(Text, primary_key=True)
    account_id: Mapped[str] = mapped_column(Text, nullable=False)
    schema_version: Mapped[str] = mapped_column(Text, nullable=False)
    report_status: Mapped[str] = mapped_column(Text, nullable=False)
    source_data_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    report_json: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)


class FollowerSnapshot(Base):
    """One observed follower count per account, source, and UTC day."""

    __tablename__ = "follower_snapshots"
    __table_args__ = (
        UniqueConstraint("account_id", "source", "snapshot_date", name="uq_follower_snapshots_account_source_day"),
        Index("ix_follower_snapshots_account_observed", "account_id", "observed_at"),
    )
...
class CreatorTrendResult(Base):
    ...
    weekly_opportunity_json: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
