...
def valid_api_key(monkeypatch: pytest.MonkeyPatch) -> str:
    # App uses depends(verify_api_key). We need to bypass or provide a valid key.
    # The actual implementation of verify_api_key checks BRAND_API_KEY env var
    # Let's set it to "test_key"
    monkeypatch.setenv("BRAND_API_KEY", "test_key")
    return "test_key"

def test_manual_campaign_match(client: TestClient, valid_api_key: str):
    mock_creators = [
        {
...
def test_ai_campaign_discover(client: TestClient, valid_api_key: str):
    mock_creators = [
        {
    ...
    with patch("backend.app.api.campaign_routes.parse_campaign_prompt", return_value=parsed_brief), \
...
def test_get_creator_lookalikes(client: TestClient, valid_api_key: str):
    mock_lookalikes = [
...
def test_get_creator_lookalikes_not_found(client: TestClient, valid_api_key: str):
