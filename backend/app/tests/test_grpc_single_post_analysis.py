...
@pytest.mark.asyncio
async def test_rest_single_post_analysis_falls_back_inline_when_enqueue_fails(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def _enqueue_fails(_payload: dict[str, Any]) -> dict[str, str]:
        raise RuntimeError("queue unavailable")
    ...
    monkeypatch.setattr(post_analysis_routes, "run_single_post_analysis_inline", _inline_succeeds)
