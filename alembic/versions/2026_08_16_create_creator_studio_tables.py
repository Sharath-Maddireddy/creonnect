"""Create Creator Studio shoot, culling, grade, and export tables.

Revision ID: 20260816_creator_studio
Revises: 20260814_imgcmp
...
revision: str = "20260816_creator_studio"
down_revision: Union[str, None] = "20260814_imgcmp"
...
def upgrade() -> None:
    ...
    if inspector.has_table("creator_studio_shoots"):
        return
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Text(), primary_key=True),
        sa.Column("shoot_file_id", sa.Text(), sa.ForeignKey("creator_studio_shoot_files.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("ix_creator_studio_culling_flags_shoot_file_id", "creator_studio_culling_flags", ["shoot_file_id"])
    ...
    op.create_index("ix_creator_studio_signature_grades_account_id", "creator_studio_signature_grades", ["account_id"])
    ...
    op.create_table(
        ...
        sa.Column("platform_profiles_json", sa.JSON(), nullable=False),
...
def downgrade() -> None:
    op.drop_table("creator_studio_export_bundles")
    op.drop_table("creator_studio_signature_grades")
    op.drop_table("creator_studio_culling_flags")
    op.drop_table("creator_studio_shoot_files")
    op.drop_table("creator_studio_shoots")
